...
const projects = [
  { id: '01', title: 'Serenity Touch NYC', type: 'WEB EXPERIENCE', image: '/images/serenity-hero.png', fallback: '/images/banner.png', secondary: '/images/serenity-team.png', description: 'A conversion-focused website for a New York in-home massage business. I shaped the experience around trust, premium presentation, clear services and a direct path to booking.', tags: ['Next.js', 'Responsive design', 'Conversion UX'], link: 'https://serenity-touch-nyc.vercel.app/' },
  { id: '02', title: 'IBARFFS COOPs', type: 'FULL-STACK PLATFORM', image: '/images/ibarffs-mobile-app.jpg', fallback: '/images/dashboard.jpg', secondary: '/images/admin.png', description: 'A connected cooperative management platform covering member onboarding, authentication, savings, shares, loans, repayments, ledgers and administration.', tags: ['Node.js', 'Prisma', 'PostgreSQL'], link: 'https://github.com/micheal-driod' },
  { id: '03', title: 'IBARFFS Admin', type: 'OPERATIONS SYSTEM', image: '/images/admin.png', fallback: '/images/dashboard.jpg', description: 'The administrative control layer for the cooperative platform, with member operations, approvals, loan review, monthly posting and protected workflows.', tags: ['React', 'REST APIs', 'Role-based access'], link: 'https://github.com/micheal-driod' },
  { id: '04', title: 'LinkQio', type: 'MOBILE PRODUCT', image: '/images/Linkqio-app.png', fallback: '/images/banner.png', description: 'A student-focused mobile product concept built around campus discovery, everyday utility and community interactions.', tags: ['Flutter', 'Mobile UX', 'Product UI'], link: 'https://github.com/micheal-driod' },
];
...
function ProjectVisual({ src, fallback, alt }: { src: string; fallback: string; alt: string }) {
  const [image, setImage] = useState(src);
  return <Image src={image} alt={alt} width={1920} height={1080} onError={() => setImage(fallback)} />;
}

function SecondaryVisual({ src }: { src: string }) {
  const [image, setImage] = useState(src);
  return <Image src={image} alt="Additional project view" width={1920} height={1080} onError={() => setImage('/images/banner.png')} />;
}
    ...
    <main className="portfolio" id="top">
      ...
      <section className="hero">
        ...
        <div className="hero-portrait">
          ...
          <div className="portrait-wrap"><ProjectVisual src="/images/segun.jpg" fallback="/images/profile.jpg" alt="Segun Michael Adegbite" /></div>
      ...
      <section className="work" id="work">
        ...
        <div className="project-list">{projects.map((p, i) => <article className={`project ${i % 2 ? 'reverse' : ''}`} key={p.id}><div className={`project-image ${p.id === '01' ? 'serenity-visual' : ''}`}><ProjectVisual src={p.image} fallback={p.fallback} alt={`${p.title} project screenshot`} /><span className="project-index">{p.id}</span><span className="project-view">VIEW PROJECT ↗</span>{p.secondary && <div className="project-secondary"><SecondaryVisual src={p.secondary} /></div>}</div><div className="project-copy"><span className="project-type">{p.type}</span><h3>{p.title}</h3><p>{p.description}</p><div className="tags">{p.tags.map(t => <span key={t}>{t}</span>)}</div><a href={p.link} target={p.link.startsWith('http') ? '_blank' : undefined} rel="noreferrer">Explore project <b>↗</b></a></div></article>)}</div>
      </section>

      <section className="story"><div className="section-label">04 / HOW I WORK</div><div className="story-head"><h2>From <i>problem</i><br />to product.</h2><p>I like clarity. Before I write a lot of code, I want to understand the person using it, the business rule behind it and what success looks like.</p></div><div className="process"><div><span>01</span><h3>Understand</h3><p>Requirements, users, constraints and the real problem.</p></div><div><span>02</span><h3>Shape</h3><p>Structure the experience, data and technical approach.</p></div><div><span>03</span><h3>Build</h3><p>Interface, APIs, authentication, database and integrations.</p></div><div><span>04</span><h3>Deliver</h3><p>Test, debug, deploy, refine and keep improving.</p></div></div></section>

      <section className="capabilities" id="capabilities"><div className="section-label">05 / CAPABILITIES</div><div className="cap-head"><h2>Useful across<br /><i>the stack.</i></h2><p>Hiring me means you can hand me a feature, a product, an existing codebase or a messy technical problem and we can work toward a finished result.</p></div><div className="service-list">{services.map(([n,t,d]) => <div className="service" key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p><b>↗</b></div>)}</div><div className="toolkit"><span>TOOLKIT</span><div>{stack.map(s => <b key={s}>{s}</b>)}</div></div></section>

      <section className="contact"><div className="contact-orbit" aria-hidden="true" /><span className="section-label">06 / CONTACT</span><h2>Let&apos;s make<br /><i>something useful.</i></h2><p>If you&apos;re hiring remotely, launching a product, improving an existing system or simply have an idea that needs a developer, I&apos;d like to hear about it.</p><div className="contact-actions"><a className="button dark" href="mailto:adegbitesegunmic02@gmail.com">Email Segun ↗</a><a href="https://github.com/micheal-driod" target="_blank" rel="noreferrer">GitHub ↗</a><a href="https://www.linkedin.com/in/adegbite-segun-59517416a/" target="_blank" rel="noreferrer">LinkedIn ↗</a></div><footer><span>SEGUN MICHAEL ADEGBITE</span><span>IBADAN, NIGERIA · WORKING WORLDWIDE</span><span>© 2026</span></footer></section>
    </main>
  );
}
